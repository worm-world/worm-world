...
const CrossNodeForm = (props: iCrossNodeForm): JSX.Element => {
  ...
  selectedVariations.forEach((selectedGene) => {
    variationsForNode.push(selectedGene.value);
  });
  const allelesForNode: Allele[] = [];
  selectedAlleles.forEach((selectedGene) => {
    allelesForNode.push(selectedGene.value);
  });
  const onSubmit = (): void => {
